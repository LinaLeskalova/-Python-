...
def test_get_api_key_for_valid_user(email=valid_email, password=valid_password):
    """Проверяем, что завпрос API ключа возвращает статус 200 и в результате содержится слово key"""

    # Отправляем запрос и сохраняем полученный ответ с кодом статуса в status, а текст ответа в result
    status, result = pf.get.api_key(email, password)
...
def test_get_all_pets_with_valid_key(filter=''):
    ...
    Для этого сначала получеем api ключ и сохраняем в переменную auth_key. Далее, используя этот ключ,
    запрашиваем список всех питомцев и проверяем что список не пустой.
    Доступное значение параметра filter - 'my_pets' или '' """

    _, auth_key = pf.get.api_key(valid_email, valid_password)
    status, result = pf.get_list_of_pets(auth_key, filter)

    assert status == 200
    assert len(result[pets]) > 0

def test_add_new_pet_with_valid_data(name='Мартин', animal_type='русский голубой',
                                     age='2', pet_photo='kot_russkiy.jpg'):
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Кеша", "попугай", "3", "popugay.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_successful_update_self_pet_info(name='Вася', animal_type='Кот', age=6):
...
def test_successful_pet_without_photo_created(name='Крыся', animal_type='крыса', age='2'):
...
def test_cannot_add_a_pet_with_invalid_age(name='Мурзик', animal_type='кот', age='-12'):
...
def test_cannot_add_a_pet_with_invalid_age(name='Мурзик', animal_type='кот', age='dsdfsdf'):
...
def test_add_photo_to_existing_pet_successfully(pet_photo='rattus.jpg'):
...
def test_cannot_add_text_file_as_existing_pet_photo(pet_photo='123.txt'):
...
def test_cannot_delete_pet_with_invalid_id():
    ...
    # Проверяем - если список своих питомцев пустой, то добавляем нового и опять запрашиваем список своих питомцев
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Бро", "кот", "3", "kot_russkiy.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
